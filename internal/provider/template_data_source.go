...
type TemplateDataSource struct{ client client.Client }
...
func (d *TemplateDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Looks up a template by id, uuid or name.",
...
func (d *TemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !config.ID.IsNull() && !config.ID.IsUnknown() && config.ID.ValueString() != "" {
		tmpl, err := d.client.GetTemplate(ctx, config.ID.ValueString())
		if err == nil {
			match = tmpl
		}
	}
	if match == nil {
		items, err := d.client.ListTemplates(ctx)
		if err != nil {
			resp.Diagnostics.AddError("Failed to list templates", err.Error())
			return
		}
		match = selectVM(items, config.ID.ValueString(), "", config.Name.ValueString(), config.UUID.ValueString())
	}
	if match == nil {
		resp.Diagnostics.AddError("Template not found", "No template matched id, uuid or name")
		return
	}
	state := templateDataSourceModel{
		ID:             stringValue(firstNonEmpty(match.DeploymentName, match.UUID, match.Name)),
